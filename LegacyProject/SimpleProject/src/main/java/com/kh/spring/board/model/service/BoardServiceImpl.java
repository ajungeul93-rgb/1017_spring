...
public class BoardServiceImpl implements BoardService {
	...
	private void validateContent(BoardDTO board) {
		...
		if(board.getBoardTitle().contains("이승철 바보")) {
			boardTitle = board.getBoardTitle().replaceAll("이승철 바보", "글쓴사람바보");
		}
	...
	public int insertReply(ReplyDTO reply, HttpSession session) {
		...
		if(loginMember == null) {
			throw new AuthenticationException("본거같아");
		}
