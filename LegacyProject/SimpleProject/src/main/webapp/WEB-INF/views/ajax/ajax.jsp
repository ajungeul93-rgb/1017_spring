...
<body>
	...
	<pre>
		...
		* 핵심!! => 비동기통신 => 부분갱신 => 사용자 경험 향상 => 트래픽 줄어듬
		--------------------------------------------------------------
		
		전송 데이터 형식 => 과거에는 XML 사용 => 현재 JSON
		AJAX 구현 AIP == XMLHttpRequest => modern Fetch API
									   => ajax() => jQuery
									   => axios() => React
		...
		2. 서버는 요청 처리 후 데이터 응답(문자열 => JSON형태)
	...
	<!-- 
		계획 : 인풋요소에 아무거나 쓰고 요청보내기 버튼을 누르면 AJAX요청을 보내서 요청을 받아서
		처리해주는 RequestHandler가 값을 받아서 응답을 해주고 받은 응답데이터를 라벨요소 Content영역에 출력
	 -->
	...
	<!-- 
		계획 : 글 번호를 입력받고, 댓글 내용을 입력받은 뒤
			 버튼을 글릭하면 AJAX요청을 보내 Reply테이블에 한 행 INSERT
	 -->
